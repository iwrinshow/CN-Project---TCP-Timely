...
NS_LOG_COMPONENT_DEFINE("GfcDumbbell");
...
{
  uint32_t qSize = 0;//queue->GetCurrentSize ().GetValue ();

  // check queue size every 1/100 of a second
  Simulator::Schedule (Seconds (0.00001), &CheckQueueSize, queue);
...
{
  LogComponentEnable("TcpTimely", LOG_LEVEL_INFO);
  uint32_t stream = 1;
  std::string transport_prot = "TcpTimely";
  ...
  cmd.AddValue ("transport_prot", "Transport protocol to use: TcpNewReno, "
                "TcpHybla, TcpHighSpeed, TcpHtcp, TcpVegas, TcpTimely, TcpScalable, TcpVeno, "
  ...
  Config::SetDefault (queue_disc_type + "::MaxSize", QueueSizeValue (QueueSize ("335p")));
